...
const ProductsDisplay = ({ product }) => {
    const {_id, title, description, price, thumbnail, stock, rating, discountPercentage } = product;
        ...
        <div>
            <div className="max-w-xs rounded-lg shadow-lg bg-slate-200 text-gray-900 relative">
                ...
                <div className="flex flex-col justify-between p-3 space-y-2">
                    ...
                    <Link to={`/checkout/${_id}`}><button className="rapiit-btn">Buy Now</button></Link>
                </div>
            </div>
        </div>
    );
